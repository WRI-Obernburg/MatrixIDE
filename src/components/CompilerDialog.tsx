...
export function CompilerDialog(props: {
    ...
    return <Card className={"bg-card shadow flex flex-col items-stretch rounded-lg m-5"}>
        ...
        {props.showDownloadButton ? <div className={"flex flex-row flex-1 gap-2 ml-4 mr-4 mb-4"}><Button className={"flex-grow"} onClick={props.downloadCompiledProgram}>Download Compiled
            Program</Button>
            <EmulatorComponent/>
        </div> : null}
    </Card>;
